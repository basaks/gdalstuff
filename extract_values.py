...
def usage():
    '''Show usage synopsis.
    '''
    print('Usage: extract_values.py [-q] [-r] [-g] [-f] point_shapefile'
          ' [raster_file(s)] [-d directory_with_rasters] [-rl list,of,rasters] '
          '[-e extension]')
    sys.exit(1)
    ...
    for i in infos:
        if i.bands == 1:
            ...
            for b in range(i.bands):
                ...
                fieldDef.SetWidth(18)
                fieldDef.SetPrecision(8)
                if fields_descript: fields_csv.write(
                    i.fileBaseName + ';' + shortName + str(b + 1) + ';' + str(b) + '\n')
                if create_csv:
...
class gdalInfo:
    ...
    def getSupportedRasters(self):
        ...
        for i in range(gdal.GetDriverCount()):
            ...
            if longName != '':
                if len(extensions) > 0:
                    # XXX add check for SDTS; in that case we want (*C.DDF)
...
if __name__ == '__main__':
    ...
    if fields_descript:
        ...
        for i in range(layerDefinition.GetFieldCount()):
            fields_csv.write(
                layerDefinition.GetFieldDefn(i).GetName() + ';' + layerDefinition.GetFieldDefn(i).GetName() + ';1\n')
