...
@click.option('-v', '--value', is_flag=True, help='Whether to sample base on '
                                                  'values')
@click.option('-s', '--space', is_flag=True,
              help='Whether to sample base on spatial distances')
@click.option('-r', '--order', required=False, type=bool, default=True,
              help='Order of sampling, value based first by default.')
@click.option('-p', '--target', required=True,
              help='The property based on which quantiles will be calculated')
def resample_shapefile(input_shp, output_shp, target,
                       value, space, validation_file=None,
                       validation_points=100):
    if (not value) and (not space):
        raise AttributeError('At least one of value or space need to be True')

    final_shpfile = abspath(output_shp)
    _transforms = []
    if value:
        _transforms.append('value')
    if space:
        _transforms.append('space')

    for i, r in enumerate(_transforms):
        _shpfile = final_shpfile if i == 1 \
            else tempfile.mktemp(suffix='.shp', dir='/tmp')

        input_shpfile = input_shp if i == 0 else out_shpfile

        # just create the validation shape during last sampling step
        validation = validation_file \
            if i == len(_transforms) - 1 else None

        out_shpfile = resampling_techniques[r](
